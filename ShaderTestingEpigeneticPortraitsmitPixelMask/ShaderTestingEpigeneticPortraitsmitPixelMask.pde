...
void setup() {
  ...
  smooth = new float[mask.pixels.length];
  smooth2 =new float[mask.pixels.length];
  for (int i = 0; i<mask.pixels.length; i++) {
    ranS[i] = random(0, 70);
    smooth[i] = random(0, 1);
    smooth2[i] = random(0,1);
  }
...
void draw() {
  ...
  for (int x = 0; x<copy.width; x++) {
    for (int y = 0; y<copy.height; y++) {
      ...
      if (grow) { 
        if (smooth[loc]<maxPix) { 
          smooth[loc]+=0.00000020;
        } else {
          grow=false;
        }
      } else {
      ...
      if (copy.pixels[loc]<0) {
        ...
        ellipse(x, y, ranS[loc]*smooth[loc], ranS[loc]*smooth[loc]);
      }
    }
  }
  ...
  for (int x = 0; x<copy2.width; x++) {
    for (int y = 0; y<copy2.height; y++) {
      ...
      //controls size of blip 
      if (grow2) { 
        if (smooth2[loc]<maxPix) { 
          smooth2[loc]+=0.00000020;
        } else {
          grow2=false;
        }
      } else {
      ...
      if (copy2.pixels[loc]<0) {
        ...
        ellipse(x, y, ranS[loc]*smooth2[loc], ranS[loc]*smooth2[loc]);
      }
    }
  }
