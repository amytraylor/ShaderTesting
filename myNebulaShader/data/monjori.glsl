...
void main(void) {
  ...
  //vec2 uv_not = -1.0 + 2.0 * gl_FragCoord.xy / resolution.xy; 
  ...
  h = ((f+d)+a/2.0)*g;
  i = cos(h+r*uv.x/1.3)*(e+e+a)+cos(q*g*6.0)*(r+h/3.0);
  h = sin(f*g)*144.0-sin(e*g)*212.0*uv.x;
  ...
  gl_FragColor = vec4((col*0.05)+(monjori*0.95), 0.5);
